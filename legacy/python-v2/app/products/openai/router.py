...
from .console import is_console_basic_model
from .schemas import (
...
async def chat_completions_endpoint(req: ChatCompletionRequest, request: Request):
    ...
    try:
        ...
        if spec.is_image_edit():
            ...
            from .console import maybe_create_chat_completion

            result = await maybe_create_chat_completion(
                model=req.model,
                messages=messages,
                stream=is_stream,
                emit_think=emit_think if emit_think is not None else cfg.get_bool("features.thinking", True),
                tools=req.tools,
