...
class SqlAccountRepository:
    ...
    async def _ensure_columns(self, conn: Any) -> None:
        ...
        if "quota_console" not in existing:
            await conn.execute(sa.text(f"ALTER TABLE {_TBL_ACCOUNTS} ADD COLUMN quota_console TEXT"))
            await conn.execute(sa.text(f"UPDATE {_TBL_ACCOUNTS} SET quota_console = '{{}}' WHERE quota_console IS NULL"))
