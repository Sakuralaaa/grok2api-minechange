...
type accountImportProviderDTO struct {
	Created int `json:"created"`
	Updated int `json:"updated"`
}
...
func (h *Handler) importFile(c *gin.Context, web bool) {
	...
	byProvider := make(map[string]accountImportProviderDTO, len(result.ByProvider))
	for key, value := range result.ByProvider {
		byProvider[key] = accountImportProviderDTO{Created: value.Created, Updated: value.Updated}
	}
	_ = stream.Write("complete", accountImportResponse{Created: result.Created, Updated: result.Updated, Synced: syncResult.Succeeded, SyncFailed: syncResult.Failed, ByProvider: byProvider})
}
...
func (h *Handler) batchRefreshTokens(c *gin.Context) {
	...
	if request.Provider != string(accountdomain.ProviderBuild) || !h.validateProviderIDs(c, ids, request.Provider) {
		if request.Provider != string(accountdomain.ProviderBuild) {
			response.Error(c, http.StatusBadRequest, "invalidProvider", "仅 Grok Build 账号支持批量刷新凭据")
		}
		return
	}
