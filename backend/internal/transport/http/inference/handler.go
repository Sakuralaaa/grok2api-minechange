...
func (h *Handler) handleCreate(c *gin.Context, compact bool) {
	...
	requestID, _ := c.Get(middleware.RequestIDKey)
	requestIDValue, _ := requestID.(string)
	input := gateway.Input{RequestID: requestIDValue, ClientKey: clientKey, PublicModel: request.Model, Body: body, Streaming: request.Stream, PromptCacheKey: request.PromptCacheKey, PromptCacheSeed: extractPromptCacheSeed(c.Request.Header, body), PreviousResponseID: request.PreviousResponseID}
...
func writeGatewayError(c *gin.Context, err error) {
	...
	case errors.As(err, &upstreamFailure):
		...
		writeRetryAfterHeader(c, upstreamFailure.RetryAfter)
...
func selectionErrorResponse(c *gin.Context, failure *gateway.SelectionUnavailableError) (int, string, string) {
	...
	writeRetryAfterHeader(c, failure.RetryAfter)
	return status, code, message
}
