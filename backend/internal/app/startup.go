...
func (a *Application) reconcileStartup(ctx context.Context) {
	...
	if count, err := a.accounts.EnsureConsoleQuotaWindows(recoveryCtx); err != nil {
		a.logger.Warn("console_quota_backfill_failed", "error", err)
	} else if count > 0 {
		a.logger.Info("console_quota_backfill_completed", "count", count)
	}
	for _, providerValue := range []accountdomain.Provider{accountdomain.ProviderBuild, accountdomain.ProviderWeb, accountdomain.ProviderConsole} {
