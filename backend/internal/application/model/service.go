...
func (s *Service) ListEnabled(ctx context.Context) ([]modeldomain.Route, error) {
	values, err := s.models.ListEnabled(ctx)
	if err != nil {
		return nil, err
	}
	out := make([]modeldomain.Route, 0, len(values))
	for _, value := range values {
		if value.Provider == account.ProviderConsole && !consoleprovider.IsListedPublic(value.PublicID) {
			continue
		}
...
func (s *Service) Create(ctx context.Context, input CreateInput) (modeldomain.Route, error) {
	...
	if !modeldomain.ValidatePublicSuffix(publicID, input.Provider) {
		return modeldomain.Route{}, invalidInput("publicId ???????? -web/-build/-console ?? provider ??")
	}
...
func (s *Service) Update(ctx context.Context, id uint64, input UpdateInput) (modeldomain.Route, error) {
	...
	if input.PublicID != nil {
		...
		if !modeldomain.ValidatePublicSuffix(publicID, value.Provider) {
			return modeldomain.Route{}, invalidInput("publicId ???????? -web/-build/-console ?? provider ??")
		}
...
func validateProviderCapability(providerValue account.Provider, capability modeldomain.Capability) error {
	...
	if providerValue == account.ProviderBuild && capability != modeldomain.CapabilityResponses {
		return invalidInput("Grok Build ??? responses ??")
	}
	if providerValue == account.ProviderConsole && capability != modeldomain.CapabilityResponses && capability != modeldomain.CapabilityChat {
		return invalidInput("Grok Console ??? responses/chat ??")
	}
	if providerValue == account.ProviderWeb && capability == modeldomain.CapabilityResponses {
		return invalidInput("Grok Web ??? responses ??")
	}
	return nil
}
