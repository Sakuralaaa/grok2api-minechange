...
// SetLegacyAPIKeys configures plaintext API keys accepted for migration compatibility.
func (s *Service) SetLegacyAPIKeys(keys []string) {
	...
	s.legacyMu.Lock()
	s.legacyKeys = normalized
	s.legacyMu.Unlock()
}

func (s *Service) matchLegacyKey(raw string) (clientkeydomain.Key, bool) {
	s.legacyMu.RLock()
	defer s.legacyMu.RUnlock()
	for _, key := range s.legacyKeys {
		if subtle.ConstantTimeCompare([]byte(raw), []byte(key)) == 1 {
			return clientkeydomain.Key{
				ID:            0,
				Name:          "legacy-api-key",
				Prefix:        "legacy",
				Enabled:       true,
				RPMLimit:      int(s.defaultRPM.Load()),
				MaxConcurrent: int(s.defaultMax.Load()),
			}, true
		}
	}
...
func (s *Service) Authenticate(ctx context.Context, raw string) (clientkeydomain.Key, func(), error) {
	...
	prefix, ok := security.SplitClientKey(raw)
	if !ok {
		legacy, matched := s.matchLegacyKey(raw)
		if !matched {
			return clientkeydomain.Key{}, nil, ErrInvalidKey
		}
		now := time.Now().UTC()
		allowed, err := s.rateLimiter.Allow(ctx, "client:legacy:"+security.HashToken(raw), legacy.RPMLimit, now)
		if err != nil {
			return clientkeydomain.Key{}, nil, fmt.Errorf("%w: RPM ???: %v", ErrRuntimeUnavailable, err)
		}
		if !allowed {
			return clientkeydomain.Key{}, nil, ErrRateLimited
		}
		release, acquired, err := s.concurrency.Acquire(ctx, "client:legacy:"+security.HashToken(raw), legacy.MaxConcurrent)
		if err != nil {
			return clientkeydomain.Key{}, nil, fmt.Errorf("%w: ????: %v", ErrRuntimeUnavailable, err)
		}
		if !acquired {
			return clientkeydomain.Key{}, nil, ErrConcurrencyLimit
		}
		return legacy, release, nil
	}
	now := time.Now().UTC()
