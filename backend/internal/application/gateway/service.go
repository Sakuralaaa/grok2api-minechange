	...
	handleResponse:
		...
		if isRetryable(response.StatusCode) && !finalWebAntiBot {
			...
			if credential.Provider == accountdomain.ProviderBuild && lastFailure.PermanentAccountDenial {
				...
				s.selector.MarkQuotaStateChanged(credential.Provider)
				failureHandled = true
			} else if credential.Provider == accountdomain.ProviderConsole && (response.StatusCode == http.StatusUnauthorized || lastFailure.CredentialRejected) {
				_ = s.accounts.MarkReauthRequired(ctx, credential.ID, "Grok Console SSO credential rejected")
				s.selector.MarkQuotaStateChanged(credential.Provider)
				failureHandled = true
			}
