...
var (
	...
	ErrCredentialRefreshPermanent = errors.New("OAuth refresh token 已永久失效")
)
...
const (
	...
	permanentRefreshExpiredReason = "OAuth refresh token 已永久失效且 access token 已过期"
)

var (
	consoleQuotaLimit         = 150
	consoleQuotaWindowSeconds = 86_400
)

// SetConsoleQuotaDefaults updates local console window defaults used for seeding and recovery.
func SetConsoleQuotaDefaults(limit, windowSeconds int) {
	if limit > 0 {
		consoleQuotaLimit = limit
	}
	if windowSeconds > 0 {
		consoleQuotaWindowSeconds = windowSeconds
	}
}
...
type ImportResult struct {
	...
	Updated    int
	AccountIDs []uint64
	ByProvider map[string]ImportProviderCount
}

// ImportProviderCount breaks import results down by provider for dual-seed feedback.
type ImportProviderCount struct {
	Created int
	Updated int
}
...
func (s *Service) persistImportedSeeds(ctx context.Context, seeds []provider.CredentialSeed, observer ImportedAccountObserver, progress BatchProgressObserver) (ImportResult, error) {
	result := ImportResult{AccountIDs: make([]uint64, 0, len(seeds)), ByProvider: map[string]ImportProviderCount{}}
	if progress != nil {
	...
	for start := 0; start < len(seeds); start += credentialImportChunkSize {
		...
		for index, value := range stored {
			...
			result.ByProvider[providerKey] = pc
		}
	}
...
func (s *Service) MarkReauthRequired(ctx context.Context, id uint64, reason string) error {
	...
	if value.Provider == accountdomain.ProviderBuild && value.EncryptedRefreshToken != "" && !value.RefreshPermanent {
		now := s.now()
		if err := s.accounts.UpdateCredentialRefreshFailure(ctx, id, value.RefreshFailureCount, now, "reauth_recovery_pending", false); err != nil {
...
func (s *Service) refreshWebQuotaMode(ctx context.Context, id uint64, mode string) (accountdomain.QuotaWindow, error) {
	...
	if value.Provider == accountdomain.ProviderConsole && (mode == "" || mode == ConsoleQuotaMode) {
		return s.refreshConsoleQuotaMode(ctx, value)
	}
	if value.Provider != accountdomain.ProviderWeb {
...
func (s *Service) refreshConsoleQuotaMode(ctx context.Context, value accountdomain.Credential) (accountdomain.QuotaWindow, error) {
	now := s.now()
	windows, err := s.accounts.GetQuotaWindows(ctx, []uint64{value.ID})
	if err != nil {
		return accountdomain.QuotaWindow{}, err
	}
	var current *accountdomain.QuotaWindow
	for index := range windows[value.ID] {
		if windows[value.ID][index].Mode == ConsoleQuotaMode {
			current = &windows[value.ID][index]
			break
		}
	}
	if current != nil && current.Remaining > 0 {
		return *current, nil
	}
	if current != nil && current.ResetAt != nil && current.ResetAt.After(now) {
		return *current, nil
	}
	resetAt := now.Add(time.Duration(consoleQuotaWindowSeconds) * time.Second)
	synced := now
	window := accountdomain.QuotaWindow{
		AccountID: value.ID, Mode: ConsoleQuotaMode, Remaining: consoleQuotaLimit, Total: consoleQuotaLimit,
		WindowSeconds: consoleQuotaWindowSeconds, ResetAt: &resetAt, SyncedAt: &synced, Source: accountdomain.QuotaSourceDefault, UpdatedAt: now,
	}
	if err := s.accounts.SaveQuotaWindows(ctx, value.ID, accountdomain.WebTierBasic, now, []accountdomain.QuotaWindow{window}); err != nil {
		return accountdomain.QuotaWindow{}, err
	}
	return window, nil
}
...
func (s *Service) QueueWebQuotaRefresh(id uint64, mode string) {
	...
	if id == 0 || (mode != "" && mode != "weekly" && mode != ConsoleQuotaMode && !isWebChatQuotaMode(mode)) {
		return
	}
...
func (s *Service) persistSeed(ctx context.Context, seed provider.CredentialSeed) (accountdomain.Credential, bool, error) {
	...
	if err == nil {
		s.WakeCredentialRefresh()
		if stored.Provider == accountdomain.ProviderConsole {
			if ensureErr := s.ensureConsoleQuotaWindow(ctx, stored.ID); ensureErr != nil {
				s.logger.Warn("console_quota_seed_failed", "account_id", stored.ID, "error", ensureErr)
			}
		}
	}
...
// ensureConsoleQuotaWindow seeds or refreshes the local console.x.ai daily window.
func (s *Service) ensureConsoleQuotaWindow(ctx context.Context, accountID uint64) error {
	if accountID == 0 {
		return nil
	}
	now := s.now()
	windows, err := s.accounts.GetQuotaWindows(ctx, []uint64{accountID})
	if err != nil {
		return err
	}
	for _, window := range windows[accountID] {
		if window.Mode == ConsoleQuotaMode {
	...
	synced := now
	window := accountdomain.QuotaWindow{
		AccountID: accountID, Mode: ConsoleQuotaMode, Remaining: consoleQuotaLimit, Total: consoleQuotaLimit,
		WindowSeconds: consoleQuotaWindowSeconds, ResetAt: &resetAt, SyncedAt: &synced, Source: accountdomain.QuotaSourceDefault, UpdatedAt: now,
	}
	return s.accounts.SaveQuotaWindows(ctx, accountID, accountdomain.WebTierBasic, now, []accountdomain.QuotaWindow{window})
}

// EnsureConsoleQuotaWindows backfills missing local windows for all enabled console accounts.
func (s *Service) EnsureConsoleQuotaWindows(ctx context.Context) (int, error) {
	ids, err := s.accounts.ListEnabledAccountIDs(ctx, accountdomain.ProviderConsole, false)
	if err != nil {
		return 0, err
	}
	count := 0
	for _, id := range ids {
		has, err := s.accounts.HasQuotaWindows(ctx, id)
		if err != nil {
			return count, err
		}
		if has {
			// still ensure console mode exists
			windows, err := s.accounts.GetQuotaWindows(ctx, []uint64{id})
			if err != nil {
				return count, err
			}
		...
		if err := s.ensureConsoleQuotaWindow(ctx, id); err != nil {
			return count, err
		}
		count++
	}
