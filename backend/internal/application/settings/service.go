...
func applyDomainConfig(base config.Config, value settingsdomain.Config) config.Config {
	...
	base.Provider.Console = config.ConsoleProviderConfig{
		...
		UserAgent: value.ProviderConsole.UserAgent, EnableSearchTools: value.ProviderConsole.EnableSearchTools,
...
func mergeEditable(current config.Config, input EditableConfig) (config.Config, error) {
	...
	next.Provider.Console.EnableSearchTools = input.ProviderConsole.EnableSearchTools
	if input.ProviderConsole.TimeoutSeconds > 0 {
		next.Provider.Console.TimeoutSeconds = input.ProviderConsole.TimeoutSeconds
	}
	if input.ProviderConsole.QuotaLimit > 0 {
		next.Provider.Console.QuotaLimit = input.ProviderConsole.QuotaLimit
	}
	if input.ProviderConsole.QuotaWindowSeconds > 0 {
		next.Provider.Console.QuotaWindowSeconds = input.ProviderConsole.QuotaWindowSeconds
	}
	// Explicit 0 disables heartbeats; negative values are ignored (keep previous/default).
	if input.ProviderConsole.StreamHeartbeatInterval >= 0 {
		next.Provider.Console.StreamHeartbeatInterval = input.ProviderConsole.StreamHeartbeatInterval
	}
