...
// DoPinnedHTTPS sends an HTTPS request to an already validated IP while
// preserving the original hostname for Host and TLS certificate verification.
...
func newPinnedHTTPSClient(proxyURL, serverName string, tlsConfig *tls.Config) (*http.Client, error) {
	...
	transport := &http.Transport{
		Proxy: nil, DialContext: direct.DialContext, ForceAttemptHTTP2: true, DisableKeepAlives: true,
