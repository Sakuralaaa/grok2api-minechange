...
type CompatConfig struct {
	LegacyAPIKeys []string `yaml:"legacyAPIKeys"`
}
...
func (c Config) Validate() error {
	...
	if strings.TrimSpace(c.Provider.Console.ResponsesURL) != "" {
		if u, err := url.ParseRequestURI(strings.TrimSpace(c.Provider.Console.ResponsesURL)); err != nil || u.Scheme == "" || u.Host == "" {
			return errors.New("provider.console.responsesURL ????? URL")
		}
	}
	if c.Provider.Console.TimeoutSeconds != 0 && (c.Provider.Console.TimeoutSeconds < 30 || c.Provider.Console.TimeoutSeconds > 3600) {
		return errors.New("provider.console.timeoutSeconds ??? 30 ? 3600 ??")
	}
	if c.Provider.Console.QuotaLimit != 0 && (c.Provider.Console.QuotaLimit < 1 || c.Provider.Console.QuotaLimit > 100000) {
		return errors.New("provider.console.quotaLimit ??? 1 ? 100000 ??")
	}
	if c.Provider.Console.QuotaWindowSeconds != 0 && (c.Provider.Console.QuotaWindowSeconds < 60 || c.Provider.Console.QuotaWindowSeconds > 7*86400) {
		return errors.New("provider.console.quotaWindowSeconds ??? 60 ? 7 ???")
	}
	if c.Provider.Console.StreamHeartbeatInterval < 0 || c.Provider.Console.StreamHeartbeatInterval > 300 {
		return errors.New("provider.console.streamHeartbeatInterval must be between 0 and 300")
	}
