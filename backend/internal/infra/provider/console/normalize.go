...
func parseConsoleRateLimitMetadata(body []byte) *provider.RateLimitMetadata {
	for _, text := range consoleRateLimitTexts(body) {
		if metadata := parseConsoleRateLimitText(text); metadata != nil {
			return metadata
		}
	}
	return nil
...
func appendConsoleRateLimitText(value string, texts *[]string) {
...
func parseConsoleRateLimitText(text string) *provider.RateLimitMetadata {
	...
	return &provider.RateLimitMetadata{
		Scope: scope, TeamID: consoleRateLimitTeamID(text), Model: consoleRateLimitModel(text),
		Actual: actual, Limit: limit, RetryAfter: retryAfter,
	}
}
