...
func (a *Adapter) ParseImportedCredentials(data []byte) ([]provider.CredentialSeed, error) {
	...
	for index, entry := range entries {
		...
		consoleName := name
		if !strings.Contains(strings.ToLower(consoleName), "console") {
			consoleName = "Grok Console " + security.HashToken(token)[:8]
		}
		result = append(result, provider.CredentialSeed{
			Provider: account.ProviderConsole, AuthType: account.AuthTypeSSO, WebTier: account.WebTierBasic,
			Name: consoleName, SourceKey: "console-sso:" + security.HashToken(token), AccessToken: token,
		})
	}
	return result, nil
}
