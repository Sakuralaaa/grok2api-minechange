...
func NewRegistry(adapters ...Adapter) *Registry {
	...
	for _, adapter := range adapters {
		...
		if !providerValue.IsValid() {
			registry.issues = append(registry.issues, fmt.Errorf("Provider %q 无效", providerValue))
			continue
		}
		...
		if source, ok := adapter.(DefinitionAdapter); ok {
			...
			if definition.Provider == "" {
				definition.Provider = providerValue
			}
			if definition.ModelNamespace == "" {
				definition.ModelNamespace = providerValue.ModelNamespace()
			}
			registry.definitions[providerValue] = definition
		} else {
			registry.issues = append(registry.issues, fmt.Errorf("Provider %s 未实现 DefinitionAdapter", providerValue))
		}
	}
	return registry
}
