...
type modelRouteModel struct {
	...
	Provider      string    `gorm:"size:32;not null;check:chk_model_routes_provider,provider IN ('grok_build','grok_web','grok_console')"`
	UpstreamModel string    `gorm:"size:255;not null;check:chk_model_routes_upstream_model,length(trim(upstream_model)) BETWEEN 1 AND 255"`
