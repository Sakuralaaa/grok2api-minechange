...
func discoveredRouteDefaults(provider account.Provider, upstreamModel string) (string, model.Capability) {
	if provider == account.ProviderBuild {
		return model.EnsurePublicSuffix(upstreamModel, account.ProviderBuild), model.CapabilityResponses
	}
	if provider == account.ProviderConsole {
		return model.EnsurePublicSuffix(upstreamModel, account.ProviderConsole), model.CapabilityResponses
	}
	if provider != account.ProviderWeb {
		return model.EnsurePublicSuffix(upstreamModel, provider), model.CapabilityResponses
	}
	switch upstreamModel {
	case "grok-imagine-image", "grok-imagine-image-quality":
		return model.EnsurePublicSuffix(upstreamModel, account.ProviderWeb), model.CapabilityImage
	case "imagine-image-edit":
		return model.EnsurePublicSuffix("grok-imagine-image-edit", account.ProviderWeb), model.CapabilityImageEdit
	case "grok-imagine-video":
		return model.EnsurePublicSuffix(upstreamModel, account.ProviderWeb), model.CapabilityVideo
	default:
		return model.EnsurePublicSuffix(upstreamModel, account.ProviderWeb), model.CapabilityChat
	}
}
