...
const endpoints: Record<string, EndpointDefinition> = {
  "chat/completions": {
    ...
    response: { id: "chatcmpl_example", object: "chat.completion", model: "grok-chat-fast-web", choices: [{ index: 0, message: { role: "assistant", content: "HTTP streaming sends response data incrementally." }, finish_reason: "stop" }], usage: { prompt_tokens: 24, completion_tokens: 12, total_tokens: 36 } },
  },
  "chat/responses": {
    ...
    response: { id: "resp_example", object: "response", status: "completed", model: "grok-chat-fast-web", output: [{ type: "message", role: "assistant", status: "completed", content: [{ type: "output_text", text: "HTTP streaming sends response data incrementally." }] }], usage: { input_tokens: 18, output_tokens: 12, total_tokens: 30 } },
  },
  "chat/messages": {
    ...
    response: { id: "msg_example", type: "message", role: "assistant", model: "grok-chat-fast-web", content: [{ type: "text", text: "HTTP streaming sends response data incrementally." }], stop_reason: "end_turn", stop_sequence: null, usage: { input_tokens: 18, output_tokens: 12 } },
  },
  "image/generations": {
  ...
  "video/get": {
    ...
    request: () => undefined,
    response: { status: "done", model: "grok-imagine-video-web", progress: 100, video: { url: "https://example.com/generated.mp4", duration: 8, respect_moderation: true } },
  },
};
...
function fallbackModel(key: string): string {
  if (key.startsWith("image/")) return key === "image/edits" ? "grok-imagine-image-edit-web" : "grok-imagine-image-web";
  if (key.startsWith("video/")) return "grok-imagine-video-web";
