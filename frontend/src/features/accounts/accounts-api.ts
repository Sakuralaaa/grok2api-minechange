...
export type AccountDTO = {
  ...
  linkedProvider?: "grok_build" | "grok_web" | "grok_console";
  createdAt: string;
  billing?: BillingDTO;
  quota: QuotaDTO;
...
export type AccountSummaryDTO = {
  ...
  recovering: number;
  attention: number;
  providers: Record<"grok_build" | "grok_web" | "grok_console", { total: number; available: number }>;
...
export type AccountImportResultDTO = {
  ...
  synced: number;
  syncFailed: number;
  byProvider?: Partial<Record<"grok_build" | "grok_web" | "grok_console", { created: number; updated: number }>>;
};
...
export function importAccounts(files: readonly File[], onProgress?: (value: AccountTaskProgressDTO) => void, signal?: AbortSignal): Promise<AccountImportResultDTO> {
  ...
  return runAccountTask("/api/admin/v1/accounts/import", body, ["created", "updated", "synced", "syncFailed", "byProvider"], onProgress, signal);
}
...
export function importWebAccounts(files: readonly File[], onProgress?: (value: AccountTaskProgressDTO) => void, signal?: AbortSignal): Promise<AccountImportResultDTO> {
  const body = new FormData();
  files.forEach((file) => body.append("files", file, file.name));
  return runAccountTask("/api/admin/v1/accounts/web/import", body, ["created", "updated", "synced", "syncFailed", "byProvider"], onProgress, signal);
}
...
export function refreshAccountsBilling(ids: string[], provider: "grok_build" | "grok_web" | "grok_console"): Promise<{ succeeded: number; failed: number }> {
...
export function deleteAccounts(ids: string[], provider: "grok_build" | "grok_web" | "grok_console"): Promise<{ deleted: number }> {
