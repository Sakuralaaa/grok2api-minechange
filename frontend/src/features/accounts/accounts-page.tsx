...
export function AccountsPage() {
  ...
  const [provider, setProvider] = useState<"grok_build" | "grok_web" | "grok_console">("grok_build");
  ...
  const importMutation = useMutation({
    mutationFn: (files: File[]) => {
      ...
      return provider === "grok_web" || provider === "grok_console" ? importWebAccounts(files, onProgress, controller.signal) : importAccounts(files, onProgress, controller.signal);
    },
    onSuccess: (result) => {
      if (importToastRef.current !== null) toast.dismiss(importToastRef.current);
      ...
      {
        const web = result.byProvider?.grok_web;
        const consoleP = result.byProvider?.grok_console;
        if (web && consoleP) {
          toast.success(t("accounts.importedDual", { created: result.created, updated: result.updated, webCreated: web.created, webUpdated: web.updated, consoleCreated: consoleP.created, consoleUpdated: consoleP.updated, synced: result.synced, syncFailed: result.syncFailed }));
        } else {
  ...
  function changeProvider(value: "grok_build" | "grok_web" | "grok_console") {
    setProvider(value);
    setPage(1);
    setSelected(new Set());
    setTypeFilter("");
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
        <AccountMetricPanel icon={<Users />} loading={summaryLoading} label={t("accounts.totalAccounts")} value={summaryUnavailable ? "-" : formatNumber(totalAccounts, i18n.language, 0)} detail={t("accounts.providerAccountBreakdown", { build: formatNumber(buildSummary.total, i18n.language, 0), web: formatNumber(webSummary.total, i18n.language, 0), console: formatNumber(consoleSummary.total, i18n.language, 0) })} />
        <AccountMetricPanel icon={<CircleCheck />} loading={summaryLoading} label={t("accounts.availableAccounts")} value={summaryUnavailable ? "-" : formatNumber(activeAccounts, i18n.language, 0)} detail={t("accounts.providerAvailableBreakdown", { build: formatNumber(buildSummary.available, i18n.language, 0), web: formatNumber(webSummary.available, i18n.language, 0), console: formatNumber(consoleSummary.available, i18n.language, 0) })} />
      ...
      <div className="space-y-6">
        <div className="mb-3 grid gap-2 sm:grid-cols-3">
          {[
            { key: "grok_build" as const, label: t("models.providerGrokBuild"), value: buildSummary },
            { key: "grok_web" as const, label: t("models.providerGrokWeb"), value: webSummary },
            { key: "grok_console" as const, label: t("models.providerGrokConsole"), value: consoleSummary },
          ].map((item) => (
            <button key={item.key} type="button" onClick={() => changeProvider(item.key)} className={`rounded-lg border px-3 py-2 text-left transition-colors ${provider === item.key ? "border-primary bg-primary/5" : "hover:bg-muted/40"}`}>
              <div className="text-xs font-medium">{item.label}</div>
              <div className="mt-1 text-sm tabular-nums"><span className="font-semibold text-emerald-600 dark:text-emerald-400">{item.value.available}</span><span className="text-muted-foreground"> / {item.value.total} {t("accounts.channelAvailable")}</span></div>
            </button>
          ))}
        </div>
        <Tabs value={provider} onValueChange={(value) => changeProvider(value as "grok_build" | "grok_web" | "grok_console")}>
          <TabsList>
        ...
        <input
          ...
          type="file"
          multiple
          accept={provider === "grok_web" || provider === "grok_console" ? "application/json,text/plain,.json,.txt" : "application/json,.json"}
          className="hidden"
          onChange={(event) => {
        ...
        {accountsQuery.isPending || (result && result.items.length > 0) ? (
          <Table className="table-fixed border-collapse min-w-[780px] xl:min-w-[960px] 2xl:min-w-[1080px]">
            <colgroup>
              ...
              <col style={{ width: "7%" }} />
              <col style={{ width: "7%" }} />
              <col style={{ width: provider === "grok_web" || provider === "grok_console" ? "46%" : "30%" }} />
            ...
            <TableBody>
              {accountsQuery.isPending ? <TableLoadingRow colSpan={provider === "grok_web" || provider === "grok_console" ? 7 : 8} /> : result?.items.map((account) => {
                ...
                return (
                  <TableRow className="group" key={account.id} data-state={selected.has(account.id) ? "selected" : undefined}>
                    ...
                    <TableCell className="text-center whitespace-nowrap">{provider === "grok_web" || provider === "grok_console" ? <WebAccountType tier={account.webTier} /> : <AccountType quota={account.quota} />}</TableCell>
                    <TableCell className="text-center whitespace-nowrap"><AccountStatus account={account} /></TableCell>
                    <TableCell>{provider === "grok_web" || provider === "grok_console" ? <WebQuota windows={account.quotaWindows ?? []} locale={i18n.language} tier={account.webTier} /> : <AccountQuota quota={account.quota} billing={account.billing} locale={i18n.language} />}</TableCell>
