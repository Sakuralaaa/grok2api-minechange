...
import { channelBadgeClass, channelLabelKey } from "@/shared/lib/channel";
...
export function ClientKeysPage() {
  ...
  const modelsQuery = useQuery({
    ...
    queryFn: () => listModels({ page: modelOptionsPage, pageSize: 100, search: debouncedModelOptionsSearch, sortBy: "provider", sortOrder: "asc" }),
            ...
            <fieldset className="space-y-2">
              ...
              <div className="overflow-hidden rounded-md border">
                ...
                <div className="max-h-52 divide-y overflow-y-auto overscroll-contain">
                  {modelsQuery.isPending ? <LoadingState className="min-h-24" /> : (() => {
                    const groups: Record<string, NonNullable<typeof modelsQuery.data>["items"]> = { grok_web: [], grok_build: [], grok_console: [] };
                    for (const model of modelsQuery.data?.items ?? []) {
                      const key = model.provider in groups ? model.provider : "grok_build";
                      groups[key].push(model);
                    }
                    return (["grok_web", "grok_build", "grok_console"] as const).map((provider) => {
                      const items = groups[provider] ?? [];
                      if (items.length === 0) return null;
                      return (
                        <div key={provider} className="border-b last:border-b-0">
                          <div className="sticky top-0 z-[1] flex items-center gap-2 bg-muted/70 px-3 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground backdrop-blur">
                            <span className={`rounded border px-1.5 py-0.5 normal-case ${channelBadgeClass(provider)}`}>{provider === "grok_web" ? "-web" : provider === "grok_console" ? "-console" : "-build"}</span>
                            {t(channelLabelKey(provider))}
