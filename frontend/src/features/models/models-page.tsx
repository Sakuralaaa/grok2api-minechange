...
import { MoreHorizontal, Pencil, Search, Trash2, Copy } from "lucide-react";
...
import { channelBadgeClass, channelLabelKey, copyText, ensurePublicSuffix, validatePublicSuffix } from "@/shared/lib/channel";
...
export function ModelsPage() {
  ...
  const [providerFilter, setProviderFilter] = useState<"grok_build" | "grok_web" | "grok_console" | "">("");
  ...
  const schema = z.object({
    ...
    .refine((value) => validatePublicSuffix(value.publicId, value.provider), { path: ["publicId"], message: t("models.publicIdSuffixMismatch") });
  ...
  const updateMutation = useMutation({
    mutationFn: (values: ModelForm) => {
      ...
      const input = { ...values, publicId: ensurePublicSuffix(values.publicId.trim(), values.provider), accountIds: values.bindingMode ? values.accountIds : [] };
                ...
                <TableRow className="group" key={model.id} data-state={selected.has(model.id) ? "selected" : undefined}>
                  ...
                  <TableCell className="min-w-0">
                    <div className="flex min-w-0 items-center gap-1.5">
                      <span className="block min-w-0 truncate text-xs font-medium" title={model.publicId}>{model.publicId}</span>
                      <Badge variant="outline" className={`shrink-0 text-[10px] ${channelBadgeClass(model.provider)}`}>{model.provider === "grok_web" ? "-web" : model.provider === "grok_console" ? "-console" : "-build"}</Badge>
                      <Button type="button" variant="ghost" size="icon" className="h-6 w-6 shrink-0 opacity-0 group-hover:opacity-100" title={t("common.copy")} onClick={() => { void copyText(model.publicId).then((ok) => ok ? toast.success(t("common.copied")) : toast.error(t("errors.generic"))); }}>
                  ...
                  <TableCell className="text-center"><Badge variant="outline">{t(channelLabelKey(model.provider))}</Badge></TableCell>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <Select value={selectedProvider} disabled>
                    ...
                    <SelectContent><SelectItem value="grok_build">{t("models.providerGrokBuild")}</SelectItem><SelectItem value="grok_web">{t("models.providerGrokWeb")}</SelectItem><SelectItem value="grok_console">{t("models.providerGrokConsole")}</SelectItem></SelectContent>
                  </Select>
                </div>
                <div className="space-y-2">
