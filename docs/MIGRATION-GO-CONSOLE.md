...
GROK2API_POSTGRES_DSN=postgres://USER:PASSWORD@HOST:5432/DBNAME?sslmode=require
```

也兼容 `DATABASE_URL` / `POSTGRES_DSN`（许多云平台默认注入这个）。

可选连接池：

```text
GROK2API_POSTGRES_MAX_OPEN_CONNS=50
GROK2API_POSTGRES_MAX_IDLE_CONNS=10
```

使用 Postgres 后，业务数据在数据库里；仍建议挂载 `/app/data` 用于媒体文件。

Zeabur 示例：同项目加 Postgres 服务，把其 `DATABASE_URL` 绑到本服务，并设置 `GROK2API_DATABASE_DRIVER=postgres`。
...
database:
  driver: postgres
  postgres:
    dsn: "postgres://user:password@host:5432/grok2api?sslmode=require"
    maxOpenConns: 50
    maxIdleConns: 10
```
